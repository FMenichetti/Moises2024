...
namespace Moises2024.Tests
{
    public abstract class Moises2024TestBase : AbpIntegratedTestBase<Moises2024TestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected Moises2024TestBase()
        {
        ...
        protected void UsingDbContext(Action<Moises2024DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<Moises2024DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<Moises2024DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<Moises2024DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<Moises2024DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Moises2024DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<Moises2024DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Moises2024DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<Moises2024DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<Moises2024DbContext, Task<T>> func)
        {
            T result;
